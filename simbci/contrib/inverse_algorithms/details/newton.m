...
% Modified by P. Schimpf 10-05-02
% to exit gracefully when too many iterations, passing back -1 as error code
...
while (abs(step) > thr*lambda & abs(step) > thr & it < it_max), it = it+1; 
  ...
  if (ps==1) 
    ...
    r = (1-f).*(beta - sigmakkk.*omega); 
    z = f.*r; 
  end
  % PHS 02-01-06, add error recovery here
  if (z'*r == 0)
      lambda = -1 ;
      return ;
  end
...
% PHS 10-05-02:  changed to warning
if (abs(step) > thr*lambda & abs(step) > thr) 
  % warning(['Max. number of iterations (',num2str(it_max),') reached'])
  lambda=-1 ;
end
