function X = champagne(G, Vmes, number_sources, number_baseline_samples, options)
	% FIXME interface above doesn't match convention
%               num_sources = size(modelExtractor.modelSourcePos, 1);
%               options.num_iterations = 70; % Arbitrary!
%               % Number of samples used to compute signal average, ASK AXELLE
%               % she does not know!
%               % FIXME : This will probably break the pipeline!
%               number_baseline_samples = 10;


    Lp = get_3D_leadfield(G, size(G,2)/number_sources, number_sources);
    ...
    flags.gamma = 'auto';  % Undocummented
    flags.champ.ax = 2;    % Undocummented
    flags.champ.multf = 0; % Undocummented
    flags.champ.nem_ch = num_iterations; % Number of iteration
    % To keep the same output size we repeat the first number_baseline_samples samples in Vmes
    Vmes = [Vmes(:, 1:number_baseline_samples), Vmes];

    data.nt = size(Vmes, 2);
    data.y = Vmes';
    flags.tb.timeptc(1) = 1; %time before stimulation
    flags.tb.timeptc(2) = number_baseline_samples; % end of time before stimulation
    flags.tb.timepta(1) = flags.tb.timeptc(2)+1; %
    flags.tb.timepta(2) = data.nt;
    data.latency = zeros(size(data.y,1),1);
    %data.latency(flags.tb.timepta(1):flags.tb.timepta(2)) = 1;
    champ_out = nut_Champagne(Lp,data,flags);
    X1 = champ_out.sources;
    X = get_2D_Mat(X1, size(X1, 1));
end
...
%data - uses data.y for sensor data: (time x sensors)
...
%col_norm=flags.cn; I commented this, 18/01/2016 (Matlab says it's unused)
AX=flags.champ.ax;
...
%nt=size(data.y(flags.tb.timeptc(1):flags.tb.timepta(2)),1); Originnally,
%changed to (on the st February) :
nt=size(data.nt);
ns=size(data.y,2);
nv=size(Lp,3);
...
nl=10; %num of factors
nem_sefa=nl;
%[a1,b,lam,alp,bet,xbar1]=sefa0(poststim,prestim,nl,25,nem_sefa,0);
[a1,b,lam,alp,bet,xbar1]=sefa0(poststim,prestim,nl,floor(0.8*size(data.y,2)),nem_sefa,0); %1/02/2016
